...
// If you wish to use an alternate SAL annotation, define SITELOCK_USE_ALTERNATE_SAL_ANNOTATIONS
// and provide the mapping below.
#ifndef SITELOCK_USE_ALTERNATE_SAL_ANNOTATIONS
#include <sal.h>
#else
// Alternate SAL annotation
#define _In_opt_count_(c)  
#define _In_z_             
#define _In_count_(c)      
#define _Inout_z_count_(c) 
#define _Out_z_cap_(c)     
#endif
...
// Version information
#define SITELOCK_VERSION 0x00010015 // 1.15
...
{(IObjectSafetySiteLock::SiteList::SiteListCategory)0,0,0}};								\
	*dwCount = _countof(rgslTrustedSites) - 1;												\
...
public:
	...
	bool InApprovedDomain(_In_opt_count_(cTrustedSites) const IObjectSafetySiteLock::SiteList * rgslTrustedSites = T::GetSiteLockMap(), int cTrustedSites = T::GetSiteLockMapCount())
#else
	// Checks if the activation URL is in an allowed domain / zone
	bool InApprovedDomain(_In_opt_count_(cTrustedSites) const IObjectSafetySiteLock::SiteList * rgslTrustedSites = T::rgslTrustedSites, int cTrustedSites = _countof(T::rgslTrustedSites))
#endif
	{
...
private:
	...
	bool FApprovedDomain(_In_z_ const OLECHAR * wzUrl, DWORD dwZone, _In_count_(cTrustedSites) const IObjectSafetySiteLock::SiteList * rgslTrustedSites, int cTrustedSites)
	{
		// Declarations
		HRESULT hr                                          = S_OK;
	...
	HRESULT NormalizeDomain(_Inout_z_count_(cchDomain) OLECHAR * wzDomain, int cchDomain)
	{
		// Data validation
		if (!wzDomain)
			return E_POINTER;
...
cleanup:
	...
	HRESULT GetDomainAndScheme(_In_z_ const OLECHAR * wzUrl, _Out_z_cap_(cchScheme) OLECHAR * wzScheme, DWORD cchScheme, _Out_z_cap_(cchDomain) OLECHAR * wzDomain, DWORD cchDomain)
	{
		// Data validation
	...
	bool MatchDomains(_In_z_ const OLECHAR * wzTrustedDomain, _In_z_ const OLECHAR * wzOurDomain)
	{
		// Data validation
		if (!wzTrustedDomain)
